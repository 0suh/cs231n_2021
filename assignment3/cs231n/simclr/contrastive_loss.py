...
def sim(z_i, z_j):
    ...
    zn_i = z_i / torch.linalg.vector_norm(z_i)
    zn_j = z_j / torch.linalg.vector_norm(z_j)
    norm_dot_product = torch.dot(zn_i, zn_j)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2 * N):
            if j != k:
                d_left += torch.exp(sim(z_k, out[j]) / tau)
            if j != N + k:
                d_right += torch.exp(sim(z_k_N, out[j]) / tau)
        loss_left = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / d_left)
        loss_right = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / d_right)
